...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        boxes, lbls, diffs = self._load_ann(img_id, scale)
        if self.flip_prob and random.random() < self.flip_prob:
            ...
            if boxes.numel() > 0:
                ...
                boxes[:, 0] = width - x2
                boxes[:, 2] = width - x1
        return img_t, boxes, lbls, diffs, img_id
    ...
    def _load_ann(self, img_id, scale):
        ...
        for obj in ET.parse(ann_path).getroot().findall("object"):
            ...
            lbls.append(CLASS_TO_IDX[name])
            diffs.append(is_diff)
        if boxes:
            return (
                torch.tensor(boxes, dtype=torch.float32),
                torch.tensor(lbls, dtype=torch.long),
                torch.tensor(diffs, dtype=torch.bool)
            )
        return (
            torch.zeros((0, 4), dtype=torch.float32),
            torch.zeros(0, dtype=torch.long),
...
def collate_fn(batch):
    imgs, boxes_l, lbls_l, diffs_l, ids = zip(*batch)
    ...
    for img in imgs:
        ...
        padded.append(pad)
        shapes.append((h, w))
    return torch.stack(padded), shapes, list(boxes_l), list(lbls_l), list(diffs_l), list(ids)
