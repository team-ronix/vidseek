...
class RPN(nn.Module):
    ...
    def forward(self, feat, img_shape, gt_boxes=None):
        ...
        props, sc = props[keep], obj_scrs[keep]
    ...
    def _loss(self, anchors, cls_t, reg_t, gt_boxes, img_shape):
        ...
        if len(gt_boxes) == 0:
            ...
            lbls[idx_in[max_iou < self.neg_iou]] = 0
            lbls[idx_in[max_iou >= self.pos_iou]] = 1
            if ious.numel() > 0:
        ...
        if len(neg_i) > num_neg:
            lbls[neg_i[torch.randperm(len(neg_i))[num_neg:]]] = -1
        valid = lbls >= 0
        Ncls = valid.sum().clamp(min=1)
        cls_loss = F.cross_entropy(cls_t[valid], lbls[valid], reduction='sum') / Ncls.float()
        pos_mask = lbls == 1
        Nreg = len(anchors) // self.k
        if pos_mask.sum() > 0 and len(gt_boxes) > 0:
            pos_a = anchors[pos_mask]
            _, best = iou_matrix(pos_a, gt_boxes).max(1)
            tgt = bbox_trans(pos_a, gt_boxes[best])
