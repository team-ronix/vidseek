...
def _triplet_match(
    pred_triplet: Tuple,   # (subj_label, pred_label, obj_label, subj_box, obj_box)
...
def mean_recall_at_k(
    ...
    pred_hits: Dict[str, int] = defaultdict(int)
...
def zero_shot_recall_at_k(
    predictions: List[List[Dict]],
    ground_truths: List[List[Dict]],
    seen_triplets: set,   # set of (subj, pred, obj) tuples seen in training
    k: int   = 50,
) -> float:
    zs_preds: List[List[Dict]] = []
    zs_gts: List[List[Dict]] = []
    for preds, gts in zip(predictions, ground_truths):
...
def evaluation_report(
    predictions: List[List[Dict]],
    ground_truths: List[List[Dict]],
    seen_triplets: Optional[set] = None,
) -> Dict:
